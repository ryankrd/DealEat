...
using DealEat.WebApp.Models.AccountViewModels;
...
namespace DealEat.WebApp.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                Result<int> result = await _userService.CreateUser(model.Email,model.Name, model.LastName,model.Pseudo, model.Telephone, model.Password);
                if (result.HasError)
                {
